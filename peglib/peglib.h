...
public:
    ...
    std::weak_ptr<Ope> get_rule() const;
...
private:
    ...
    mutable std::weak_ptr<Ope>                         rule_;
};
...
struct AssignIDToDefinition : public Ope::Visitor
{
    ...
    void visit(DefinitionReference& ope) override { ope.get_rule().lock()->accept(*this); }
...
inline size_t DefinitionReference::parse(
    ...
    const auto& rule = *get_rule().lock();
    return rule.parse(s, n, sv, c, dt);
}

inline std::weak_ptr<Ope> DefinitionReference::get_rule() const {
    if (!rule_.lock()) {
