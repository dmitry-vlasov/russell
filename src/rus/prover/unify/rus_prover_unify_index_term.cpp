	...
	string show(bool full = false) const {
		...
		if (full) {
			...
			if (indexIter.isValid()) {
				oss << "\t" << n << "-iter: " << indexIter.show() << endl;
			} else {
				oss << "\t" << n << "-iter: NOT VALID " << indexIter.show() << endl;
			}
		}
		oss << "Term iter:" << endl;
		if (termIter.isValid()) {
			oss << "\t" << n << "-iter: " << termIter.show() << endl;
		} else {
			oss << "\t" << n << "-iter: NOT VALID " << termIter.show() << endl;
		}
		oss << "sub: " << endl;
	...
	if (i.equals()) {
		...
		if (data.consistent) {
			if (data.rule) {
				...
				for (auto& pair : do_unify_index_term(subBegins)) {
					try {
...
string show_unification_result(const map<uint, TermSubst>& result) {
	ostringstream oss;
	oss << "result:" << endl;
	oss << "---------" << endl;
...
void check_unification(const Index& ind, const Term& term, const map<uint, TermSubst>& result) {
