...
struct Grammar : qi::grammar<LocationIter, rus::Source(), unicode::space_type> {
	...
	static void mark_vars(Expr& ex, VarStack& var_stack) {
		for (auto& s : ex.symbols) {
			...
			if (!is_const && !is_var) {
				string msg = "symbol " + Sys::get().lex.symbols.toStr(s.lit) + " ";
	...
	inline uint create_id(string pref, string s1, string s2) {
		return Sys::mod().lex.labels.toInt(pref + "_" + s1 + "_" + s2);
	}

	inline Symbol create_symbol(string str, Type* tp) {
		return Symbol(Sys::mod().lex.symbols.toInt(str), tp, tp);
	}
	...
	struct AddToMath {
		...
		void operator()(Proof* p) const {
			...
			if (!p->has_id) p->id = Sys::mod().lex.labels.toInt(to_string(p->ind));
	...
	struct SymbToInt {
		...
		uint operator()(const std::vector<uint>& s) const {
			...
			return Sys::mod().lex.symbols.toInt(symb);
		}
	};
	...
	struct IdToInt {
		...
		uint operator()(const std::vector<uint>& id) const {
			...
			return Sys::mod().lex.labels.toInt(id_str);
		}
	};
	...
	struct FindType {
		...
		Type* operator()(uint id) const {
			...
			return Undef<uint>::is(id) ? nullptr : Sys::mod().math.types[id];
		}
	};
	...
	template<typename LocationIter>
	struct SetLocation {
		template <typename T1, typename T2>
		struct result { typedef void type; };
	...
	struct AssembleDef {
		...
		void operator()(Def* d, VarStack& varsStack) const {
			static Symbol dfm(Sys::mod().lex.symbols.toInt("defiendum"));
			static Symbol dfs(Sys::mod().lex.symbols.toInt("definiens"));
			Prop* prop = new Prop;
	...
	qi::rule<LocationIter, Symbol(), unicode::space_type> var;
	qi::rule<LocationIter, Symbol(), unicode::space_type> symb;
	qi::rule<LocationIter, uint(), unicode::space_type> id;
	qi::rule<LocationIter, string(), unicode::space_type> path;
	qi::rule<LocationIter, void(Expr&, Rule*), unicode::space_type> term;
	qi::rule<LocationIter, void(Expr&, Type*), unicode::space_type> expr;
	qi::rule<LocationIter, void(Expr&, Type*), unicode::space_type> plain;
	qi::rule<LocationIter, Disj(), unicode::space_type> disj;
	qi::rule<LocationIter, Vars(), qi::locals<Symbol>, unicode::space_type> vars;
	qi::rule<LocationIter, Hyp*(), qi::locals<uint>, unicode::space_type> hyp;
	qi::rule<LocationIter, Prop*(), qi::locals<uint>, unicode::space_type> prop;
	qi::rule<LocationIter, Ref(Proof*), unicode::space_type> ref;
	qi::rule<LocationIter, vector<Ref>(Proof*), unicode::space_type> refs;
	qi::rule<LocationIter, Step*(Proof*), qi::locals<uint, uint, Step::Kind, uint, vector<Ref>, Expr>, unicode::space_type> step;
	qi::rule<LocationIter, Qed*(Proof*), qi::locals<uint>, unicode::space_type> qed;
	qi::rule<LocationIter, Proof::Elem(Proof*), unicode::space_type> proof_elem;
	qi::rule<LocationIter, void(Proof*), unicode::space_type> proof_body;
	qi::rule<LocationIter, Proof*(), unicode::space_type> proof;
	qi::rule<LocationIter, Theorem*(), qi::locals<Assertion*>, unicode::space_type> theorem;
	qi::rule<LocationIter, Def*(), qi::locals<Assertion*, Type*>, unicode::space_type> def;
	qi::rule<LocationIter, Axiom*(), qi::locals<Assertion*>, unicode::space_type> axiom;
	qi::rule<LocationIter, void(Assertion*), unicode::space_type> assertion;
	qi::rule<LocationIter, Rule*(), unicode::space_type> rule;
	qi::rule<LocationIter, Type*(), unicode::space_type> type;
	qi::rule<LocationIter, Const*(), unicode::space_type> constant;
	...
	static bool parse(LocationIter& beg, LocationIter& end, auto space, Source& src) {
