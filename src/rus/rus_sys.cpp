...
Return prove_test(string mode, string theorem, uint max_proofs) {
	if (mode == "oracle") {
		return prover::test_with_oracle(theorem, max_proofs);
	}
	return Return();
}
...
const Sys::Actions& Sys::actions() {
	static Actions actions = {
		...
		{"prove_test",    Action([](const Args& args) { return prove_test(args[0], args[1], stoul(args[2])); }, description("prove_test"))},
