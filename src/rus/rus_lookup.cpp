...
static const Tokenable* find_ref(const Symbol& symb, const char* pos) {
	return symb.token()->includes(pos) ? symb.tokenable() : nullptr;
}

static const Tokenable* find_ref(const Vars& vars, const char* pos) {
	if (vars.token.includes(pos)) {
		for (const auto& var : vars.v) {
			if (const Tokenable* tok = find_ref(var, pos)) {
				return tok;
			}
		}
	}
...
static const Tokenable* find_ref(const Type* type, const char* pos) {
	if (type->token.includes(pos)) {
		for (const auto& sup : type->sup) {
			if (const Tokenable* tok = find_ref(sup, pos)) {
				return tok;
			}
		}
	}
...
static const Tokenable* find_ref(const Rule* rule, const char* pos) {
	if (rule->token.includes(pos)) {
		if (const Tokenable* tok = find_ref(rule->vars, pos)) {
			return tok;
		}
		if (const Tokenable* tok = find_ref(rule->term.type, pos)) {
			return tok;
		}
	}
	return nullptr;
...
static const Tokenable* find_ref(const Assertion* ass, const char* pos) {
	if (ass->token.includes(pos)) {
		if (const Tokenable* tok = find_ref(ass->vars, pos)) {
			return tok;
		}
...
static const Tokenable* find_ref(const Step* step, const char* pos) {
	if (step->token.includes(pos)) {
		if (step->kind() == Step::ASS && step->ass_token().includes(pos)) {
			return step->ass();
		}
		if (const Tokenable* tok = find_ref(step->expr.type, pos)) {
			return tok;
		}
	}
	return nullptr;
...
static const Tokenable* find_ref(const Proof::Elem& e, const char* pos) {
	switch (Proof::kind(e)) {
...
static const Tokenable* find_ref(const Proof* proof, const char* pos) {
	if (proof->token.includes(pos)) {
		if (const Tokenable* tok = find_ref(proof->allvars, pos)) {
			return tok;
		}
...
static const Tokenable* find_ref(const Theory* theory, const char* pos);

static const Tokenable* find_ref(const Theory::Node& n, const char* pos) {
...
static const Tokenable* find_ref(const Theory* theory, const char* pos) {
	if (theory->token.includes(pos)) {
		for (auto& n : theory->nodes) {
			if (const Tokenable* tok = find_ref(n, pos)) {
				return tok;
			}
		}
	}
