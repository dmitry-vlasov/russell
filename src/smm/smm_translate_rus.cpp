...
void translate_constant(const Constant* constant, Maps& state) {
...
template<typename T>
rus::Vars translate_vars(vector<T*> decls, Maps& state) {
	rus::Vars rus_vars;
	for (auto flo : decls) {
...
inline rus::Import* translate_import(const Inclusion* inc, Maps& s) {
