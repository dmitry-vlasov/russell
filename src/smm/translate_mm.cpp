...
mm::Source* translate_source(const Source* src, Maps& maps);
...
void translate(const Node& node, mm::Block* target, Maps& maps) {
	...
	case Node::ASSERTION: {
		...
		string name = Sys::get().lex.labels.toStr(ass->prop->label);
		...
		for (auto& inn : ass->inner) {
			...
			uint new_index = Sys::mod().lex.labels.toInt(label);
	...
	case Node::INCLUSION: {
		...
		mm::Inclusion* i = new mm::Inclusion(s->label);
...
mm::Source* translate_source(const Source* src, Maps& maps) {
	if (maps.sources.count(src)) {
		...
		mm::Source* target = new mm::Source(src->label);
...
mm::Source* translate_to_mm(const Source* source) {
	Maps maps;
	return translate_source(source, maps);
}
